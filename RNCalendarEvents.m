...
static NSString *const _guid = @"guid";
static NSString *const _calendar = @"calendar";
...
- (NSArray *)serializeCalendarEvents:(NSArray *)calendarEvents
{
    ...
    for (EKEvent *event in calendarEvents) {
        ...
        if (event.calendarItemExternalIdentifier) {
            [formedCalendarEvent setValue:event.calendarItemExternalIdentifier forKey:_guid];
        }

        if (event.calendar.title) {
            [formedCalendarEvent setValue:event.calendar.title forKey:_calendar];
        }
